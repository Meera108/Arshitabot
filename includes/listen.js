module.exports = function ({ api, models }) {
        ...
        if (day != day_now) {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    let checkttBody = '==ARSHITA RAJPUT ❤️==\n\n';
                ...
                if (day_now == 1) {
                    ...
                    for (const checkttFile of checkttData) {
                        ...
                        for (const item of checktt.week) {
                            const userName = await Users.getNameUser(item.id) || 'Maleficent Here';
                            const itemToPush = item;
                            itemToPush.name = userName;
                            storage.push(itemToPush);
                        };
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ Arshita Rajput ]');
    ...
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, "[ Maleficent Mistress ]");
